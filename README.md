...
* Run
  ...
  ```
  This should take less than 1 minute. If you get more than a few lines of output, then you're rebuilding Mathlib from scratch, which means that one of the steps above (`lake exe cache get!`) went wrong. You can quit the execution (by typing `ctrl c` or `cmd c`)on Windows) and ask for help.
